...
const Dashboard = () => {
    ...
    function getDate(str) {
        // Find the index of the first occurrence of a space character
        const spaceIndex = str.indexOf(" ");

        // Extract the substring from the start of the string to the space character
        const dateSubstring = str.substring(0, spaceIndex);
    ...
    const fetchLoans = () => {
        axiosClient.get("/bar_chart").then((response) => {
            setBarData(
                response.data.map((item) => {
                    return {
                        field: item.field_name,
                        loaned: parseInt(item.count_loaned),
                        loanedColor: "red",
                        available: parseInt(item.count_not_loaned),
                        availableColor: "green",
                        overdued: parseInt(item.count_due_date),
                        overduedColor: "orange",
                    };
                })
            );
            setPieData(
                response.data.map(item =>{
                    return {
                        ...
                        value: parseInt(item.count_loaned) + parseInt(item.count_not_loaned),
                    }
                })
            )
        });
                ...
                return [
                    {
                        ...
                        data: [
                            { x: "January", y: 0 },
                            ...response.data.borrowers_chart.map((item) => {
                                return {
                    ...
                    },
                    {
                        ...
                        data: [
                            { x: "January", y: 0 },
                            ...response.data.members_chart.map((item) => {
                                return {
                                    x: getMonthName(item.month),
                                    y: item.count,
                                };
